...
export const Tab = ({ label, active, onClick }: TabProps) => (
  <button
    onClick={onClick}
    className={`font-poppins px-2 md:px-4 py-2 text-sm md:text-lg  transition-colors duration-300 ${
      active
  ...
  return (
    <main className="font-poppins p-3 md:p-5">
      ...
      >
        ...
        <span className="hidden md:block">Back to Regions</span>
      </button>

      <section className="p-3 md:p-8 bg-white rounded-md ">
        <div className="border-[0.5px] border-[#71839B] rounded-md p-3 md:p-5 ">
            <h3 className="font-semibold md:text-xl text-[#454545] ">Region Details</h3>

            <div className="mt-6 flex flex-wrap justify-between">
                {churchData.map((data, idx) => (
                    <div className="w-full md:w-1/2 mb-5">
                        <p className="font-light text-[#545454] mb-1 md:mb-3">{data.label} </p>
                        <h2 className="font-semibold text-lg md:text-2xl text-[#252525] ">{data.value} </h2>
                    </div>
                ))}
